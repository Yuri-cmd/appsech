...
class ConsumoInsumosTable extends StatefulWidget {
  static GlobalKey<_ConsumoInsumosTableState> consumoInsumosTableKey =
      GlobalKey<_ConsumoInsumosTableState>();
  final int? registro;
  const ConsumoInsumosTable({super.key, this.registro});
  ...
  Future<void> _fetchControles(int recordId) async {
    try {
      List<Map<String, dynamic>> controles =
          await ApiService.fetchControles('insumo', recordId);
      setState(() {
        _data = controles;
        _isLoading = false;
      });
